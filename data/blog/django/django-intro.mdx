...
# Summary
지금까지 MVT 패턴과 URL Conf를 활용하여 장고 프레임워크의 기초적인 코드 흐름을 나름의 방식으로 분석해보았다. 지금 사용중인 장고 서버는 다양한 확장 패키지들을 추가로 설치하여 사용중이라 순수 장고의 모양은 많이 사라졌지만, 대부분의 패키지들도 장고의 기본 원리대로 작동한다. 개발 생산성을 높이기 위해 다양한 시도를 하지만, 앞선 개발자들의 다양한 시도로 프레임워크의 기본적인 방식이 적립되어 있다는 걸 잊지말자.
