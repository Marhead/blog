...
Template 파일은 *.html 확장자를 가지며, 장고의 Template 시스템 문법에 맞게 작성한다. 유의할 점은 Template 파일을 **적절한 디렉토리**에 위치시켜야 한다는 점이다. 즉, 장고에서 Template 파일을 찾는 방식을 이해하고 있어야하며, 장고는 그에 맞는 위치에 Template 파일이 위치해야 Template 파일을 찾을 수 있다.
...
# Summary
지금까지도 애용하고 있는 장고 프레임워크의 기본 요소들에 대해서 나름의 방식으로 정리해보았다. 지금은 다양한 확장 패키지들을 추가로 설치하여 사용중이라, 순수 장고의 모양은 많이 사라졌지만, 어차피 기본 원리는 똑같다. 생산성을 높이기 위해 다양한 시도를 하지만, 역시나 가장 짧은 길은 언제나 기본에 있다는 걸 잊지말고 열심히 개발하자.
